...
exit_points = [
    ...
    (35.797443, 56.882207),
    (35.932805, 56.902966),
    (56.831684, 35.804913)  # Новая точка №7
]
...
ROUTE_GROUPS: Dict[int, Dict[str, List[str]]] = {
    0: {  # Понедельник
        "РЖ_СЦ_ЗБ_ЗД_ЖК_ТЦ_ВЛ_НЛ_ОЛ_ВЛ": [
            ...
            "Нелидово", "Оленино", "Зубцов", "Ржев", "Старица", "Колталово"
        ],
        "КШ_КЗ_КГ": ["Кашин", "Калязин", "Кесова Гора"],
    },
    1: {  # Вторник
        "КВ_КЛ": ["Конаково", "Редкино", "Мокшино", "Новозавидовский", "Клин", "Вараксино"],
    ...
    4: {  # Пятница
        "ТО_СП_ВВ_БГ_УД": ["Удомля", "Вышний Волочек", "Спирово", "Торжок", "Лихославль", "Медное"],
...
def check_route_match(locality, delivery_date):
    ...
    if day_of_week not in ROUTE_GROUPS or not ROUTE_GROUPS[day_of_week]:
        return False
    for route_name, route_locations in ROUTE_GROUPS[day_of_week].items():
        if locality in route_locations:
            return True
    # Дополнительная проверка по ближайшей точке выхода
    nearest_exit = find_nearest_exit_point(geocode_address(f"{locality}, Тверская область", os.environ.get("API_KEY"))[0],
                                          geocode_address(f"{locality}, Тверская область", os.environ.get("API_KEY"))[1])[0]
    exit_coords = {
        (36.055364, 56.795587): [1],  # Клин, Конаково, Вараксино
        (35.871802, 56.808677): [1],  # Волоколамск
        (35.804913, 56.831684): [0],  # Ржев, Старица, Колталово
        (36.020937, 56.850973): [2],  # Дубна
        (35.797443, 56.882207): [4],  # Торжок, Медное
        (35.932805, 56.902966): [4],  # Сонково
        (56.831684, 35.804913): [0]   # Новая точка, связана с Ржев
    }
    if nearest_exit in exit_coords:
        for route_locations in ROUTE_GROUPS[exit_coords[nearest_exit][0]].values():
            if any(loc in distance_table and haversine(geocode_address(f"{loc}, Тверская область", os.environ.get("API_KEY"))[0],
                                                      geocode_address(f"{loc}, Тверская область", os.environ.get("API_KEY"))[1],
                                                      geocode_address(f"{locality}, Тверская область", os.environ.get("API_KEY"))[0],
                                                      geocode_address(f"{locality}, Тверская область", os.environ.get("API_KEY"))[1]) < 50
