...
# Примечания
# =====================
# 1) DISTANCE_TABLE хранит полный пробег (туда-обратно) от точки выхода.
# 2) При расчёте через ORS вычисляется маршрут от ближайшей точки выхода до адреса, затем умножается на 2.
# 3) Для адресов в пределах Твери берётся только базовая стоимость по размеру груза.
# 4) Для синонимов (Завидово/Новозавидовский) добавлен явный ALIASES и запись в таблице для Завидово.
# 5) Исключён asyncio: все запросы синхронные, чтобы избежать конфликтов с событийным циклом в средах деплоя."}
